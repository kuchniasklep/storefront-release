...
const ProductPurchase = class {
  ...
  CartHandler() {
    if (productStore.store.get("availability") > 0) {
  ...
  render() {
    ...
    return [
      index.h("button", { onClick: () => this.CartHandler(), class: "cart" + (productStore.store.get("cartLoading") ? " loading" : "") + (available ? "" : " disabled") }, available ? this.cartText : this.availabilityText, index.h("ks-loader", { oversized: true, running: this.cartAnimation })),
