import { r as registerInstance, h, g as getElement, H as Host, e as createEvent } from './index-5edb5049.js';
...
import { c as createStore } from './index-4223a160.js';
...
const NavbarCategorySimple = class {
  ...
  render() {
    ...
    return h(Host, null, h("a", { href: this.category.url }, this.category.name), 'children' in this.category ? [
...
const NavbarCategoryView = class {
  ...
  render() {
    ...
    return h(Host, null, h("a", { href: this.category.url }, this.category.name), h("ks-icon", { name: "chevron-down", size: 0.8 }), h("div", { class: "children", style: childrenstyle, hidden: this.hidden }, h("div", { class: "buttons" }, this.category.children.map((child, index) => h("a", { href: child.url, class: 'children' in child && index == this.active ? "active" : null, onMouseOver: () => 'children' in child ? this.active = index : null }, child.name, 'children' in child ? null :
      h("ks-icon", { name: "link", size: 0.65 })))), h("div", { class: "content" }, this.category.children.map((child, index) => 'children' in child ?
      h("div", { hidden: index != this.active }, child.children.map(sub => h("a", { href: sub.url }, sub.name)))
      : null)), h("div", { class: "graphic" }, this.category.children.map((category, index) => "image" in category ?
...
const Navbar = class {
  ...
  render() {
    ...
    return [
      h("nav", null, h("div", { class: "logo" }, h("div", null, h("a", { href: "/" }, h("ks-img", { contained: true, sync: true, src: common.get("logo"), width: 217, height: 35, alt: "kuchniasklep.pl" })), common.get("promo") ? common.get("promoLink") && !this.mobile ?
        h("a", { class: "promo", href: common.get("promoLink") }, common.get("promo")) :
        h("span", { class: "promo" }, common.get("promo"))
        : null)), h("div", { class: "search" }, h("ks-navbar-search", null)), h("div", { class: "buttons", id: "ks-navbar-menu-buttons" }, h("ks-navbar-button", { name: "Kontakt", icon: "phone", class: "tablet-desktop", onClick: () => this.root.querySelector("ks-navbar-contact-panel").Toggle() }), commonDynamic.get('loggedIn') ?
        h("ks-navbar-button", { name: "Twoje konto", link: common.get('accountLink'), icon: "user", class: "desktop" })
        : null, h("ks-navbar-button", { name: "Schowek", link: common.get('heartLink'), icon: "star", count: heartCount, class: "tablet-desktop" }), h("ks-navbar-button", { name: "Koszyk", link: common.get('cartLink'), icon: "shopping-bag", count: cartCount }), !commonDynamic.get('loggedIn') ?
        ...
        : null, h("ks-navbar-button", { name: "Menu", icon: "menu", class: "mobile-tablet", onClick: () => this.root.querySelector("ks-navbar-sidebar").show() })), h("ks-navbar-contact-panel", { phone: common.get('phone'), email: common.get('email'), contact: common.get('company') })),
...
const NavbarSidebar = class {
  ...
  render() {
    ...
    return h("ks-sidepanel", null, h("ul", null, h("li", { class: "header" }, "MENU"), h("li", { class: "divider" }), !commonDynamic.get("loggedIn") ?
      h("li", null, h("a", { href: common.get("loginLink") }, "Zaloguj si\u0119 ", h("ks-icon", { name: "log-in" })))
      : null, h("li", { class: "small" }, h("a", { href: common.get("heartLink") }, "Schowek", favouritesCount && favouritesCount != 0 ?
      h("span", { class: "badge" }, favouritesCount) :
      h("ks-icon", { name: "star" }))), commonDynamic.get("loggedIn") ?
      h("li", null, h("a", { href: common.get("accountLink") }, "Panel Klienta ", h("ks-icon", { name: "user" })))
      : null, commonDynamic.get("loggedIn") ?
      h("li", null, h("a", { href: common.get("logoutLink") }, "Wyloguj si\u0119 ", h("ks-icon", { name: "log-out" })))
      : null, h("li", { class: "header" }, "KATEGORIE"), h("li", { class: "divider" })), common.get('categories').map(category => h("ks-category-sidebar", { category: category })));
  }
  get root() { return getElement(this); }
};
