import { r as registerInstance, h, g as getElement, e as createEvent } from './index-7a533c43.js';
...
let ProductCalculatorPayU = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.price = "";
    this.posId = "";
...
let ProductTab$1 = class {
  ...
  render() {
    const comments = product.get('comments');
    return [
      h("div", { class: "message" }, h("ks-icon", { name: "mail", size: 2.5 }), h("p", null, comments.addMessage)),
      ...
      this.hasMore ?
        ...
        : null,
      comments.addLink ?
        h("a", { href: comments.addLink, rel: "nofollow", class: "add", "aria-label": "Napisz recenzj\u0119" }, h("ks-icon", { name: "plus-circle", size: 1.5 }))
        : null
    ];
  }
  get root() { return getElement(this); }
...
let ProductImages = class {
  ...
  render() {
    return [
      h("div", { class: "swiper-container preview" }, h("div", { class: "swiper-wrapper" }, product.get("images").map((image, index) => h("div", { class: "swiper-slide" }, h("canvas", { width: image.preview.width, height: image.preview.height }), h("ks-img", { contained: true, center: true, sync: index == 0, src: image.preview.url, width: image.preview.width, height: image.preview.height, onClick: () => this.lightbox.show(index) }))))),
      product.get("images").length > 1 ?
        h("div", { class: "swiper-container thumb" }, this.loaded ? null : h("ks-loader", { dark: true }), h("div", { class: "swiper-wrapper" }, product.get("images").map((image, index) => h("div", { class: "swiper-slide" }, h("ks-img", { sync: index < 6, contained: true, center: true, src: image.thumb.url, width: image.thumb.width, height: image.thumb.height })))))
        : null,
...
let ProductSuggestions = class {
  ...
  render() {
    return h("ks-overlay", null, h("div", { class: "content" }, h("div", { class: "top" }, h("div", { class: "heading" }, "Dodano do koszyka"), h("div", { class: "name" }, this.name), h("div", { class: "buttons" }, h("ks-button", { secondary: true, name: "Przejd\u017A do koszyka", onClick: () => this.toCart() }), h("ks-button", { name: "Przegl\u0105daj dalej", onClick: () => this.hide() }))), h("div", { class: "bottom" }, this.loading ? null : h("h3", { class: "suggestion-heading" }, this.suggestionHeading), this.loading ? h("ks-loader", { dark: true, large: true }) : null, h("div", { class: "swiper-container product-suggestions", style: { display: this.loading ? "none" : "block" } }, h("div", { class: "swiper-wrapper" }, this.products.map((product) => h("div", { class: "swiper-slide" }, h("ks-product-card", { "link-only": true, name: product.name, img: product.image, link: product.link, currentPrice: parseFloat(product.currentPrice), previousPrice: product.previousPrice != "0.00" ? parseFloat(product.previousPrice) : null }))))), h("div", { class: "fade-left" }), h("div", { class: "fade-right" }))));
  }
  get root() { return getElement(this); }
};
...
let ProductCount = class {
  ...
  render() {
    return product.get("traits").map((trait) => h("div", { class: "trait" }, h("label", null, trait.name, ":"), h("div", { class: "select" }, h("select", { onChange: () => this.traitChangeHandler() }, trait.items.map(item => h("option", { value: item.id }, item.name))), h("ks-icon", { name: "chevron-down" }))));
  }
  get root() { return getElement(this); }
};
