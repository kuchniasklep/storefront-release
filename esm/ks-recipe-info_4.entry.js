...
const RecipeInfo = class {
  ...
  render() {
    return [
      ...
      h("div", { class: "image" }, h("ks-img2", { vertical: true, src: this.image }))
    ];
  }
  link(name, url) {
    if (url)
...
const RecipeIngredient = class {
  ...
  render() {
    return [
      h("label", null, h("input", { type: "checkbox", checked: this.checked, onClick: event => this.click(event) }), h("span", { class: "checkmark" })),
...
export { RecipeInfo as ks_recipe_info, RecipeIngredient as ks_recipe_ingredient, RecipeNutrition as ks_recipe_nutrition, RecipeProcedure as ks_recipe_procedure };
