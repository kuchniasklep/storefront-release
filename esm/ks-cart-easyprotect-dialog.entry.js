...
import { s as store, e as easyprotectChange } from './cart-store-2dd755ad.js';
...
let CartEasyprotectDialog = class {
  constructor(hostRef) {
  ...
  render() {
    ...
    return [
      ...
      h("ks-overlay", null, h("div", { class: `content ${products} ${productsHide}` }, this.products()), h("div", { class: `content ${warranty} ${warrantyHide}` }, this.warranty()))
    ];
  }
  products() {
    return h("div", { class: "products" }, h("slot", { name: "products" }), h("div", { class: "items" }, this.eligible.map(item => h("ks-cart-easyprotect-product", { image: item.img, name: item.name, warranty: item.warranty, onClick: () => this.toggle(item.id), active: this.active.includes(item.id) }))), h("ks-button", { tall: true, name: "Przejd\u017A dalej", onClick: () => this.addProducts(), disabled: this.active.length <= 0 }), h("div", { class: "close", onClick: () => this.hide() }, h("ks-icon", { name: "x", size: 1.2 })));
  }
  warranty() {
    const chosen = Object.keys(store.get("easyprotect"))
      .filter(id => this.active.includes(id));
    return [
      h("div", { class: "warranty" }, h("slot", { name: "warranty" }), h("div", { class: "items" }, chosen.map(id => h("ks-cart-easyprotect-warranty", { "product-id": id }))), h("ks-button", { tall: true, name: "Dodaj gwarancj\u0119", onClick: () => this.addWarranty(), disabled: this.active.length <= 0 }), h("div", { class: "close", onClick: () => this.hide() }, h("ks-icon", { name: "x", size: 1.2 })), this.eligible.length > 1 ?
        h("div", { class: "back", onClick: () => this.back() }, h("ks-icon", { name: "chevron-left", size: 1.2 }))
        : null),
      this.step == 3 ?
        h("div", { class: "loading" }, h("ks-loader", { large: true, dark: true }))
        : null
    ];
  }
  toggle(id) {
